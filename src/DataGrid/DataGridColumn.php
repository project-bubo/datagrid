 ...
 * @version    $Id: DataGridColumn.php 18 2009-05-20 22:41:25Z mail@romansklenar.cz $
 */
abstract class DataGridColumn extends \Nette\Component implements IDataGridColumn
{
	...
	{
		if ($this->caption instanceof \Nette\Web\Html && $this->caption->title) {
	...
	{
		return $this->lookup('\DataGrid\DataGrid', TRUE)->link('order', $this->getName());
	}
	...
	{
		$filter = new Filters\CheckboxFilter();
		$this->getDataGrid(TRUE)->getComponent('filters', TRUE)->addComponent($filter, $this->getName());
		return $filter;
	}
	...
	{
		$filter = new Filters\SelectboxFilter($items, $skipFirst);
		$this->getDataGrid(TRUE)->getComponent('filters', TRUE)->addComponent($filter, $this->getName());
		return $filter;
	}
}