 ...
 * @version    $Id: DataGridRenderer.php 18 2009-05-20 22:41:25Z mail@romansklenar.cz $
 */
class DataGridRenderer extends \Nette\Object implements IDataGridRenderer
{
	...
	public function renderErrors()
	{
		...
		if (count($errors)) {
			...
			foreach ($errors as $error) {
				$item = clone $li;
				if ($error instanceof \Nette\Web\Html) {
					$item->add($error);
				} else {
					$item->setText($error);
				}
	...
	public function renderPaginator()
	{
		...
		if ($first instanceof \Nette\Web\Html) {
		...
		if ($prev instanceof \Nette\Web\Html) {
		...
		$link = clone $a->href($this->dataGrid->link('page', $paginator->page + 1));
		if ($next instanceof \Nette\Web\Html) {
		...
		if ($last instanceof \Nette\Web\Html) {
	...
	protected function generateHeaderRow()
	{
		...
		foreach ($this->dataGrid->getColumns() as $column) {
			...
			if ($column instanceof Columns\ActionColumn) $cell->class[] = 'actions';
	...
	protected function getWrapper($name)
	{
		...
		if ($data instanceof \Nette\Web\Html) return clone $data;
